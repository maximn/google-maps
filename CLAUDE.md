...
### Release Process
- **Do not edit `CHANGELOG.md` by hand in feature PRs.** It is regenerated by `release.sh` from the commit log on release, using Keep a Changelog 1.1.0 categories. Manual entries to `[Unreleased]` are redundant and will conflict with the generated content. Write good conventional-commit messages instead — that is the source of truth.
- See [`RELEASING.md`](RELEASING.md) for the release workflow (`release.sh` with `claude -p` to generate notes, version bump, tag, push).
