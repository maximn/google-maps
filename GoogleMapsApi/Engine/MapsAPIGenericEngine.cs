...
namespace GoogleMapsApi.Engine
{
        ...
        where TResponse : IResponseFor<TRequest>
    {
        ...
        internal static string DefaultReferrer = string.Empty;
        ...
        protected internal static TResponse QueryGoogleAPI(TRequest request, TimeSpan timeout, string referrer)
        {
            if (request == null)
            ...
            try
            {
                ...
                var data = new WebClientEx(timeout, referrer).DownloadData(uri);
        ...
        {
            return QueryGoogleAPIAsync(request, TimeSpan.FromMilliseconds(Timeout.Infinite), DefaultReferrer, CancellationToken.None);
        }

        protected internal static Task<TResponse> QueryGoogleAPIAsync(TRequest request, TimeSpan timeout, string referrer, CancellationToken token = default(CancellationToken))
        {
            if (request == null)
                throw new ArgumentNullException("request");
            ...
            new WebClient().DownloadDataTaskAsync(uri, timeout, referrer, token)
