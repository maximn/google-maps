...
| **B3** | medium | `DurationJsonConverter` and `OverviewPolylineJsonConverter` use reflection (`GetProperty`) without null-checks; fragile to member renames and silently accept partial objects. | `Engine/JsonConverters/DurationJsonConverter.cs`, `Engine/JsonConverters/OverviewPolylineJsonConverter.cs` | Replace reflection with direct (de)serialization of explicit DTOs, or add null-checks + tests. |
